...
class Board:
    ...
    def make_new_board(self):
        # make a new board based on dim_size and num_bombs
        # make list of lists here, but since we have a 2D board, list of lists is most natural
        ...
        while bombs_planted < self.num_bombs:
            ...
            row = loc // self.dim_size # number of times dim_size foes into loc to tell us the row we will be indexing in
    ...
    def get_num_neighboring_bombs(self, row, col):
        # let's iterate through each of the neighboring postiions and sum number of bombs
    ...
    def dig(self, row, col):
        ...
        # dig at loc
        # return True if successful dig, Flase if bomb is dug
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1), min(self.dim_size-1, col+1)+1):
                if (r, c) in self.dug:
                    continue # do not dig where you already dug
                self.dig(r, c)
        
        return True
        
    def __str__(self):
        # magic function that if you call print on this object, it prints what the function returns
...
def play(dim_size = 10, num_bombs = 10):
    # step 1: create board, plant bomb
    board = Board(dim_size, num_bombs)

    # step 2: show the user the board and ask for where they want to dig
    # step 3a: if location is a bomb, show game over message
    # step 3b: if location is not a bomb, dig recursively until each square is at least next to a bomb
    # step 4: repeat steps 2 and 3a/b until there are no more places to dig for VICTORY message
    safe = True
    ...
    # 2 ways to end loop, check for each
    if safe:
